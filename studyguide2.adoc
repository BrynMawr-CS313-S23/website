...
image:Study/triangle-strip-ex1.jpg
...
* Sketch the code for a fragment shader that colors are pixels green, e.g. (0,1,0). 

* Show how you would modify the following shader to animate the light so it revolves around the object. Assume the object is centered at the origin and has maximum size of 10 units. 
...
* Visualize the matrix stack corresponding to the following code.

* Draw the shapes corresponding to the following code.
...
* Consider the algorithm from the previous question. Add code that uses a matrix stack to place Beethoven at the steering wheel. 

* Consider the following texture. How would the texture render on the following meshes?

* Suppose we shade the point p with the Phong shading model. Is p in light or shadow? Is p part of the specular hightlight, or no? Apply the phong shading equations to justify your answer.
